...
echo "    cmd.exe /c sc create `"${ud_user_lsa_secret_task_name}`" binPath= `"C:\Windows\System32\cmd.exe`" obj= `"${ud_domain}\${ud_user_lsa_secret_username}`" password= `"${ud_user_lsa_secret_pass}`"" >> $installScriptPath
echo "" >> $installScriptPath
